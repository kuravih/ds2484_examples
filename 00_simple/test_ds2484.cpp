...
int main()
{
    DS2484 bus((DS2484_I2C_BUS_PREFIX + std::to_string(29)).c_str(), false);
    bus.scan_for_sensors();
    kato::log::cout << KATO_RED << "tester.cpp::main() : found " << std::setfill('0') << std::setw(2) << bus.samples.size() << " sensors " << KATO_RESET << std::flush;
